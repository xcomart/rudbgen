...
use crate::menu::{MenuButton, MenuEntry};
...
impl TabBar {
    ...
    /// [`ScrollbarState`](crate::scrollbar::ScrollbarState) beside the handle it
...
impl RenderOnce for TabBar {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        let tabs = self.tabs.into_iter().enumerate().map(move |(index, tab)| {
            ...
            let group = SharedString::from(format!("rudbgen-tab-{index}"));
