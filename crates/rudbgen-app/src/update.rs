...
//!
//! rudbgen is not a single file. The executable resolves `lib/rudbgen-bridge.jar`
//! and the bundled `runtime/` JRE *relative to itself* — see the JVM loader in
//! `rudbgen-jdbc` — so a new binary beside an old bridge JAR is a mismatch that
...
//! no swap at all. On macOS all three live inside `rudbgen.app`, so there the
...
//! no entry, is not an installed program, and inventing one would put rudbgen
...
//! `%LOCALAPPDATA%\Programs\rudbgen` and a zip unpacked wherever the user
...
//! upgrade list while its files stayed at the old release — a worse failure
//! than the one being fixed, because nothing afterwards corrects it. So the
//! entry's `InstallLocation` is compared against the directory this executable
//! is actually running from, and a mismatch means the entry belongs to someone
...
//! on `lib/rudbgen-bridge.jar` and on the loaded images under `runtime/`, and
...
//! user sees one flow either way: the dialog finishes and rudbgen comes back up
//! on the new build.
//!
...
//! is a working rudbgen, and keeping the directory would only turn one failure
...
//! and the only thing written outside the directory rudbgen is already
...
const LATEST_RELEASE_API: &str = "https://api.github.com/repos/xcomart/rudbgen/releases/latest";
...
const RELEASES_PAGE: &str = "https://github.com/xcomart/rudbgen/releases";
...
/// legitimately takes minutes, and rudbgen's carries a whole JRE, so it is
...
const FALLBACK_ARCHIVE: &str = "rudbgen-update";
...
/// *product*, so sharing rudbman's would have winget treat an installed
/// rudbman as an installed rudbgen and offer to upgrade one into the other.
/// The other two corners are written against this one when `packaging/`
/// arrives in M6, which is also when the test that checked the two mechanically
/// comes back — it reads `rudbgen.iss`, and there is no such file yet. It is
/// what made this constant reachable off Windows, which is why the `cfg` below
/// names that platform alone until it returns.
#[cfg(windows)]
...
/// An Intel Mac or an ARM Linux box runs a locally built rudbgen, and there is
...
#[cfg(target_os = "macos")]
const BUNDLE_EXECUTABLE: &str = "Contents/MacOS/rudbgen";
...
pub struct Asset {
    ...
    /// `rudbgen-v0.2.0-x86_64-pc-windows-msvc.zip`.
    pub name: String,
...
/// own — rudbgen is not quarantine-aware, and `tar` restores none from the
...
///
/// Always `false` until M2. The answer is "is a JVM loaded into this process",
/// and nothing in the M0 shell loads one — `rudbgen-jdbc` is the crate that
/// would, and it does not exist yet. It becomes
/// `cfg!(windows) && rudbgen_jdbc::Jvm::get().is_some()` the moment it does,
/// which is why the whole deferral path below is carried rather than cut: the
/// launch that has to apply a staged update is a Windows launch with a driver
/// loaded, and that is exactly what M2 adds.
fn must_defer() -> bool {
    false
}
...
fn download(asset: &Asset, to: &Path, report: &mut dyn FnMut(Progress)) -> Result<(), String> {
    ...
    let mut response = agent
        .get(&asset.url)
        .header("User-Agent", format!("rudbgen/{CURRENT_VERSION}"))
...
/// Everything this run of rudbgen would replace, in the order it replaces them.
///
...
/// `install.sh` puts the tree in `~/.local/share/rudbgen` and links
/// `~/.local/bin/rudbgen` at it, and this answers the real directory rather
/// than the link's.
fn install_plan() -> Result<Vec<Entry>, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        let bundle = bundle_root(&exe)
            .ok_or_else(|| "rudbgen is not running from an application bundle".to_string())?;
        Ok(vec![Entry {
            target: bundle,
            source: PAYLOAD[0],
        }])
...
///
/// `current_exe()` in a bundle is `<name>.app/Contents/MacOS/rudbgen`, but the
...
/// `rudbgen.exe` becomes `rudbgen.exe.old` and not `rudbgen.old`: the second
...
fn asset_name(tag: &str, target: &str) -> String {
    ...
    format!("rudbgen-{tag}-{target}.{extension}")
}
...
/// rejected rather than truncated — rudbgen does not publish them, so a tag
...
mod tests {
    ...
    fn a_release_response_yields_its_tag_and_page() {
        ...
        let body = r#"{
            ...
            "name": "rudbgen 0.2.0",
            "draft": false,
            "html_url": "https://github.com/xcomart/rudbgen/releases/tag/v0.2.0",
            "assets": []
        }"#;
        ...
        assert_eq!(
            release.url,
            "https://github.com/xcomart/rudbgen/releases/tag/v0.2.0"
        );
        assert_eq!(
            release_url(&release),
            "https://github.com/xcomart/rudbgen/releases/tag/v0.2.0"
    ...
    fn an_asset_name_follows_the_release_workflow() {
        ...
        assert_eq!(
            ...
            "rudbgen-v0.2.0-aarch64-apple-darwin.tar.gz"
        );
        assert_eq!(
            asset_name("v0.2.0", "x86_64-unknown-linux-gnu"),
            "rudbgen-v0.2.0-x86_64-unknown-linux-gnu.tar.gz"
        );
    }
    ...
    fn the_displaced_copy_keeps_its_whole_name() {
        for (path, expected) in [
            ("C:/Program Files/rudbgen/rudbgen.exe", "rudbgen.exe.old"),
            ("/usr/local/share/rudbgen/rudbgen", "rudbgen.old"),
            ("/usr/local/share/rudbgen/runtime", "runtime.old"),
            ("/Applications/rudbgen.app", "rudbgen.app.old"),
        ] {
    ...
    fn a_bundle_is_found_however_deep_the_binary_sits() {
        assert_eq!(
            ...
            Some(PathBuf::from("/Applications/rudbgen.app"))
        );
        // The extension is what identifies it, not the depth or the name.
        assert_eq!(
            bundle_root(Path::new("/tmp/x/Some Name.APP/Contents/MacOS/rudbgen")),
        ...
        assert_eq!(
            bundle_root(Path::new("/work/rudbgen/target/debug/rudbgen")),
            None
        );
        assert_eq!(bundle_root(Path::new("/usr/local/bin/rudbgen")), None);
    }

    #[test]
    fn an_archive_name_can_never_leave_the_staging_directory() {
        assert_eq!(
            archive_name("rudbgen-v0.2.0-x86_64-pc-windows-msvc.zip"),
            "rudbgen-v0.2.0-x86_64-pc-windows-msvc.zip"
        );
    ...
    const TRIPLE: [&str; 3] = ["rudbgen", "lib", "runtime"];
    ...
    fn the_payload_is_found_at_the_root_or_one_level_down() {
        ...
        let wrapper = root.join("rudbgen-v0.2.0-x86_64-unknown-linux-gnu");
        ...
        fs::create_dir_all(bundles.path().join("wrapper/rudbgen.app/Contents")).expect("a bundle");
        assert_eq!(
            find_payload(bundles.path(), &["rudbgen.app"]),
    ...
    fn a_directory_missing_one_entry_is_not_the_payload() {
        let root = tempfile::tempdir().expect("a temp directory");
        let wrapper = root.path().join("rudbgen-v0.2.0-x86_64-pc-windows-msvc");
        tree(&wrapper, &["rudbgen", "lib"], "new");
    ...
    fn a_deferred_update_is_parked_beside_the_installation() {
        ...
        let payload = staging.join("unpacked/rudbgen-v0.2.0-x86_64-pc-windows-msvc");
    ...
    fn a_staged_update_that_cannot_be_swapped_rolls_back_and_is_still_cleared() {
        ...
        tree(&pending, &["rudbgen", "lib", "runtime", "extra"], "new");
    ...
    fn the_relaunch_target_is_something_that_can_be_executed() {
        ...
        assert_eq!(
            exe,
            PathBuf::from("/Applications/rudbgen.app/Contents/MacOS/rudbgen")
        );
        #[cfg(not(target_os = "macos"))]
        assert_eq!(exe, PathBuf::from("/opt/rudbgen/rudbgen"));
    ...
    impl ScratchKey {
        fn new() -> Self {
            let path = format!("Software\\rudbgen-test-{}", uuid::Uuid::new_v4());
    ...
    fn an_entry_that_is_not_there_is_not_created() {
        ...
        let installed = tempfile::tempdir().expect("a temporary directory");
        let absent = format!("Software\\rudbgen-test-{}", uuid::Uuid::new_v4());
