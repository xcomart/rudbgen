...
//! [`SecretStore`](rudbgen_core::SecretStore) on save, never to
...
pub struct ConnectionDialog {
    ...
    /// The session the workspace has open, and the driver it runs through.
    ///
    /// Handed in by the shell every time the dialog opens, and passed on to
    /// the driver editor so that a custom-query test can run against a
    /// database that is already connected rather than opening one of its own
    /// (architecture document, §4.6).
    open_session: Option<(String, SessionHandle)>,
...
impl ConnectionDialog {
    ...
    /// Tells the dialog which session the workspace has open, if any.
    ///
    /// Only the driver editor uses it, and only for the **Test** button: a
    /// custom query tested against a database that is already connected costs
    /// nothing and asks no password.
    pub fn set_open_session(&mut self, session: Option<(String, SessionHandle)>) {
    ...
    fn open_driver_manager(&mut self, cx: &mut Context<Self>) {
        // Built from the form as it stands rather than from what is on disk:
        // the driver being edited is usually the one the half-finished profile
        // above is about to use, and a test against the saved URL would be a
        // test of a different database.
        let target = self.selected.is_some().then(|| {
            let profile = self.collect(cx);
            ...
            TestTarget {
                ...
                driver_id,
                profile,
                password: Some(text(&self.password_input, cx)),
            }
        });
        let manager = cx.new(|cx| {
            let mut manager = DriverManager::new(cx);
            manager.set_test_target(target);
            manager
        });
    ...
    fn render_tunnel(&self, chrome: &Theme, cx: &mut Context<Self>) -> impl IntoElement + use<> {
        ...
        let body = (enabled && self.tunnel_open).then(|| {
            ...
            // rudbgen does something with it.
    ...
    fn render_form(&self, chrome: &Theme, cx: &mut Context<Self>) -> impl IntoElement + use<> {
        ...
        let no_auth = self.current_driver().is_some_and(|driver| driver.no_auth);
        ...
        div()
            ...
            .child(
                div()
                    ...
                    .child(url)
                    // A file-backed database — SQLite, an embedded H2 — has no
                    // user and no password to ask for, and jdbgen's `noAuth`
                    // is the driver saying so. The two rows go rather than
                    // being greyed: a disabled field is a question the user is
                    // not allowed to answer, where there is no question here.
                    .when(!no_auth, |body| {
                        body.child(form_row(
                            ts!("connect.username"),
...
mod tests {
    ...
    fn the_colour_tags_are_all_parseable() {
        ...
        for color in COLORS {
            assert!(rudbgen_ui::parse_hex(color).is_some(), "{color}");
        }
