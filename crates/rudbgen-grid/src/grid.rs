...
use rudbgen_ui::text_input::TextInput;
use rudbgen_ui::theme::{Theme, theme, window_translucent};
...
impl<S: GridSource> GridView<S> {
    ...
    pub fn new(source: S, cx: &mut Context<Self>) -> Self {
        let mut grid = Self {
            ...
            v_bar_id: ElementId::from(("rudbgen-grid-vbar", cx.entity_id())),
            h_bar_id: ElementId::from(("rudbgen-grid-hbar", cx.entity_id())),
        };
        grid.ensure_layout();
        grid
    }
...
mod tests {
    ...
    fn a_row_is_marked_only_when_the_source_says_so(cx: &mut TestAppContext) {
        let (grid, mut cx) = open(Staged::new(), cx);
        let palette = cx.update(|_, cx| rudbgen_ui::theme::theme(cx));
