...
These are the manifests that put rudbgen in the Windows Package Manager, so that
`winget install Xcomart.Rudbgen` works. They live here rather than only in
...
package it can find, upgrade or uninstall. `packaging/windows/rudbgen.iss`
...
|---|---|
| `Xcomart.Rudbgen.yaml` | The version manifest — the identifier, the version, and which locale is the default |
| `Xcomart.Rudbgen.installer.yaml` | The download URL, its SHA256, the installer type and scope, and the `ProductCode` |
| `Xcomart.Rudbgen.locale.en-US.yaml` | Everything a human reads: publisher, license, description, tags |
...
```powershell
$ver = "0.1.3"
$url = "https://github.com/xcomart/rudbgen/releases/download/v$ver/rudbgen-v$ver-x86_64-pc-windows-msvc-setup.exe"
Invoke-WebRequest -Uri $url -OutFile "$env:TEMP\rudbgen-setup.exe"
(Get-FileHash -Algorithm SHA256 "$env:TEMP\rudbgen-setup.exe").Hash
```
...
```powershell
winget validate --manifest packaging\winget\0.1.3
```
...
```powershell
winget install --manifest packaging\winget\0.1.3
```
...
```powershell
$dst = "<fork>\manifests\x\Xcomart\Rudbgen\0.1.3"
New-Item -ItemType Directory -Force $dst
Copy-Item packaging\winget\0.1.3\*.yaml $dst
```
...
```
wingetcreate update Xcomart.Rudbgen --version <version> --urls <setup.exe url> --submit
```
...
`packaging/windows/rudbgen.iss`'s `AppId` with `_is1` appended, the suffix Inno
Setup adds when it writes its uninstall registry key. It is the only thing that
lets winget match an installed copy of rudbgen to this package — which is why
`AppId` in `rudbgen.iss` is frozen. Change it and every existing installation
becomes invisible to `winget upgrade` and `winget uninstall`, silently, and
installing the new version leaves the old one behind in Apps & features with no
way to remove it through winget. There is no migration path; the only fix is
never to do it.
...
**The built-in updater keeps the version in step, with a gap.** rudbgen has an
updater of its own (`crates/rudbgen-app/src/update.rs`) that fetches a release
...
months, `winget upgrade` offering a release already applied — so the updater now
writes the new version into that key itself, after a successful update. It only
ever writes to an entry whose `InstallLocation` is the directory the running
executable is actually in, so a portable copy unpacked from the zip neither
creates an entry nor touches the installed copy's.

Two edges remain, and both are narrow.

When the updater has to defer the file swap to the next launch — which it does
on Windows if a database connection has already loaded the JVM, because the JAR
and the runtime are then locked — the registry value is written straight away
while the files change on the following start. The entry therefore reads one
release ahead for as long as rudbgen stays open. In the rarer case where the
staged update turns out to be unapplicable and is discarded, it stays one ahead
until the next successful update corrects it.

**Uninstalling after several self-updates can leave files behind.** Inno's
uninstall log is written at install time and lists what the installer put down.
Files that a later self-update *added* — a new module under `runtime\`, say —
are not in that log, so the uninstaller does not remove them and leaves the
directory in place because it is not empty. Removing what is left is a manual
`Remove-Item`. Reinstalling over the top is unaffected.
