...
import comart.rudbgen.bridge.codec.BatchWriter;
import comart.rudbgen.bridge.codec.ColumnKind;
import comart.rudbgen.bridge.codec.ColumnWriter;
import comart.rudbgen.bridge.codec.ColumnWriters;
import comart.rudbgen.bridge.meta.SqlTypes;
...
public final class Cursor {
    ...
    private ColumnWriter[] newWriters(int capacity) {
        ...
        for (int i = 0; i < w.length; i++) {
            w[i] = ColumnWriters.forColumn(colTypes[i], colPrecision[i], capacity);
        }
        return w;
    }
