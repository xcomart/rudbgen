...
fn main() {
    println!("cargo::rerun-if-env-changed=RUDBGEN_BRIDGE_JAR");

    let jar = match std::env::var_os("RUDBGEN_BRIDGE_JAR") {
        Some(path) => PathBuf::from(path),
        None => workspace_jar(),
    };

    // Rebuilding when the JAR changes keeps `RUDBGEN_BRIDGE_JAR` (below) from
    ...
    if !jar.is_file() {
        panic!(
            "the Java bridge JAR is missing: {}\n\
             ...
             \n\
             or point RUDBGEN_BRIDGE_JAR at an existing rudbgen-bridge.jar.\n\
             This build script deliberately does not invoke Gradle itself.",
            jar.display()
        );
    }

    println!("cargo::rustc-env=RUDBGEN_BRIDGE_JAR={}", jar.display());
}

/// `<workspace>/bridge/build/libs/rudbgen-bridge.jar`, where Gradle puts it.
fn workspace_jar() -> PathBuf {
    ...
    workspace.join("bridge/build/libs/rudbgen-bridge.jar")
}
