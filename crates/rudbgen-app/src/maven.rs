...
fn fetch_jar(
    ...
    loop {
        ...
        if bytes.len() as u64 > MAX_BYTES {
            return Err(DownloadError::Network(format!(
                "{url} is larger than the {} MB rudbgen will download",
                MAX_BYTES / (1024 * 1024)
            )));
        }
...
/// what keep it honest. The same trade `rudbgen_jdbc::spec` makes for base64.
...
mod tests {
    ...
    /// run it with `cargo test -p rudbgen-app -- --ignored` when the downloader
