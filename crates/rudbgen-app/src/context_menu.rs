...
//! — but not as [`MenuEntry`], which is write-only: a row of it carries a boxed
//! callback and a private label, so nothing can be read back out of one. What a
//! menu offers on a given table row, a given tab or a given template is exactly
//! the decision worth testing, and a test that had to click at a computed pixel
//! to find out would be testing the menu's line height.
//!
...
//! it.
//!
//! The M0 shell draws no context menu: the surfaces that have one — the
//! explorer's table rows (M2), the work area's tabs and the template list (M3)
//! — are not on screen yet. What is here is the skeleton they build against,
//! and it is kept rather than deferred because the menu of a surface is written
//! at the same time as the surface and this is the shape it has to be written
//! in.
