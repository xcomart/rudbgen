...
fn fresh_url() -> String {
    ...
    format!(
        "jdbc:h2:mem:rudbgen{};DB_CLOSE_DELAY=-1",
...
fn the_jvm_is_one_per_process_however_often_it_is_asked_for() {
    ...
    let second = Jvm::start(&JvmConfig::new("/nonexistent/rudbgen-bridge.jar").with_heap_mb(64))
...
fn every_type_survives_execute_fetch_and_decode() {
    ...
    // rudbman answered a CLOB with a reference for a later LOB_READ. That op is
    // retired (architecture.md D3), so the text is in the batch.
    assert_eq!(batch.value(0, 11), Some(Value::Str("clob body")));
...
impl TempFile {
    fn new(name: &str, contents: &[u8]) -> TempFile {
        let path = std::env::temp_dir().join(format!("rudbgen-probe-{name}"));
