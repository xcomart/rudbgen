//! Platform-specific locations of the files rudbgen persists.
//!
...
//! Support/com.example.rudbgen`, and the architecture document pins the plain
//! `rudbgen` form.
//!
//! Most of what rudbgen persists is a single file in that directory —
//! [`settings_file`], [`connections_file`], [`drivers_file`],
//! [`template_sets_file`], [`abbreviations_file`], [`known_hosts_file`]. What
//! has no fixed number of members gets a subdirectory instead:
//! [`templates_dir`], [`ui_themes_dir`], [`editor_themes_dir`] and
//! [`drivers_dir`].
...
/// Name of the file holding the serialized [`crate::TemplateSetStore`].
const TEMPLATE_SETS_FILE_NAME: &str = "template-sets.json";

/// Name of the file holding the serialized [`crate::AbbreviationStore`].
const ABBREVIATIONS_FILE_NAME: &str = "abbreviations.json";
...
/// Name of the directory holding the template files.
...
/// here — the theme and template files the app layer reads included.
pub fn strip_bom(bytes: &[u8]) -> &[u8] {
    bytes.strip_prefix(UTF8_BOM).unwrap_or(bytes)
}

/// Resolve the project directories for rudbgen.
fn project_dirs() -> Result<ProjectDirs> {
    ProjectDirs::from("", "", APP_NAME)
        .context("could not determine a home directory for the current user")
}

/// Directory that holds every rudbgen configuration file.
///
...
/// Full path of the template set file (`template-sets.json`).
///
/// A set is a named list of templates — jdbgen's presets — and the file is one
/// list of those, so it is a file rather than a directory. The template
/// *bodies* live in [`templates_dir`]; this only names them.
///
/// # Errors
///
/// Fails when no home directory can be determined for the current user.
pub fn template_sets_file() -> Result<PathBuf> {
    Ok(config_dir()?.join(TEMPLATE_SETS_FILE_NAME))
}

/// Full path of the abbreviation rule file (`abbreviations.json`).
///
/// # Errors
///
/// Fails when no home directory can be determined for the current user.
pub fn abbreviations_file() -> Result<PathBuf> {
    Ok(config_dir()?.join(ABBREVIATIONS_FILE_NAME))
}
...
/// Directory holding the template files (`templates`).
///
/// One file per template, in whatever language the template writes. The
/// built-in set is copied here on a first run and never overwritten
/// afterwards, so a user's edit of a shipped template survives an update.
///
/// Like [`config_dir`], the directory is not created by this call.
///
/// # Errors
///
/// Fails when no home directory can be determined for the current user.
pub fn templates_dir() -> Result<PathBuf> {
    Ok(config_dir()?.join(TEMPLATES_DIR_NAME))
}
...
mod tests {
    ...
    fn config_paths_share_the_config_directory() {
        ...
        let cases: [(PathBuf, &str); 10] = [
            ...
            (
                template_sets_file().expect("template sets"),
                TEMPLATE_SETS_FILE_NAME,
            ),
            (
                abbreviations_file().expect("abbreviations"),
            ...
            (templates_dir().expect("templates"), TEMPLATES_DIR_NAME),
        ];
