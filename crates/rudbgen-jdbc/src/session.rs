...
//! (the bridge's `README.md`, "Concurrency").
//!
...
//! a channel, whatever the application prefers — is `rudbgen-app`'s job, and
...
impl Session {
    ...
    ///
    /// Every kind comes back as `{kind, items[]}`.
    ...
    /// Invokes an operation and hands back its raw OK payload.
    ///
    /// The escape hatch for a response this crate has no typed accessor for.
    /// Goes through the worker like every other command, so it stays serialised
    /// against the connection, and an ERROR envelope still becomes an
    /// [`Error`] rather than reaching the caller as bytes.
    pub fn call_raw(
        &self,
        op: Op,
        handle: i64,
...
impl Worker {
    ...
    fn start(jvm: &'static Jvm) -> Result<Arc<Worker>> {
        ...
        let thread = std::thread::Builder::new()
            .name("rudbgen-session".to_string())
...
fn run(jvm: &'static Jvm, receiver: Receiver<Command>, ready: Sender<Result<()>>) {
    ...
    let attached: Result<()> = jvm.vm().attach_current_thread(|env| {
        ...
        while let Ok(command) = receiver.recv() {
            ...
            // (the bridge's `README.md`, "Concurrency"). `AssertUnwindSafe` is honest: the
