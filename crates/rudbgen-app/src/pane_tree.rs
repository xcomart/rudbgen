...
//! tab has an identity the tree does not: which template file it holds, and
//! whether it is the one Generate tab.
//!
//! Nothing in this file is reached by the M0 shell, which draws a welcome
//! screen and no work area at all. It is here because the work area of M3 is
//! built on it and because the rules below are worth having under test before
//! anything depends on them; the module is `#[allow(dead_code)]` at its
//! declaration in `main.rs` for exactly that reason.
//!
...
//! of tab is one variant of [`PaneItem`] plus one arm where the shell renders
//! the active tab; the tree, and every test in this file, stays untouched.
//!
//! # Why a pane holds a list rather than one panel
//!
//! Editing a template while the Generate tab says what it will be run against
//! is the ordinary case, and a pane that held exactly one thing would answer it
//! by throwing the previous tab away. Tabs make the two coexist without forcing
//! the user to split the window for every template they open. An empty list is
//! a state of its own — the pane keeps standing, showing the empty state —
//! because closing the last tab must not rearrange a layout the user placed.
...
/// renderer so that it survives a repaint and being merged into another tree as
/// a subtree.
const EVEN: f32 = 0.5;
...
/// mean the ids its owner had stored — the active pane of the tree being merged
...
pub enum PaneItem {
    ...
    /// [`Pane::template_of`] matches on: opening the same template twice is a
    /// navigation to the tab that already has it, not a second buffer over one
    /// file.
    Template {
        ...
        ///
        /// The tab shows it as a marker, and it is what [`PaneItem::blocks_close`]
        /// answers from.
        dirty: bool,
    },
...
impl PaneItem {
    ...
    ///
    /// Only an unsaved template does. The Generate tab holds nothing that is
    /// not already in the connection's generation profile, and a preview is
    /// rendered to memory and can be rendered again.
    pub fn blocks_close(&self) -> bool {
        matches!(self, PaneItem::Template { dirty: true, .. })
    }
}
...
impl Pane {
    ...
    /// The index of the Generate tab, if it is in this pane.
    ///
    /// There is exactly one in the window, so this answers where it is rather
    /// than whether to open another.
    pub fn generate(&self) -> Option<usize> {
        self.items
            .iter()
            .position(|item| matches!(item, PaneItem::Generate))
    }
    ...
    pub fn template_of(&self, file: &Path) -> Option<usize> {
...
/// The panes of the work area, arranged as a binary tree.
///
/// A tree always holds at least one pane: [`PaneTree::remove`] refuses to take
/// the last one, leaving the caller to decide what an empty work area means.
#[derive(Debug)]
...
impl<T> PaneTree<T> {
    ...
    ///
    /// This is how a detached layout comes back in: its panes move over
    /// unchanged, keeping both their arrangement and their ids. Returns whether
    ...
    /// refused with `None`, because a work area with no panes would have
    /// nothing to render; the caller decides what to do instead.
    ///
...
mod tests {
    ...
    fn merging_a_subtree_keeps_its_layout_and_its_ids() {
        ...
        // The ids the source layout was tracking still name the same panes.
    ...
    fn a_removed_pane_can_be_replanted_in_a_tree_of_its_own() {
        // How detaching a pane works: remove hands the payload back, so it
    ...
    /// The tab list's own rules, which the shell leans on around every focus
    /// dance: what `push` brings to the top, where `close` leaves the marker,
    /// and that an empty pane answers every question without panicking — the
    ...
    #[test]
    fn a_new_tab_comes_to_the_top_and_closing_walks_leftwards() {
        let mut pane = Pane::new();
        pane.push(PaneItem::Generate);
        pane.push(template("a.java", false));
        pane.push(template("b.java", false));
        assert_eq!(pane.active_index(), 2, "the newest tab is the one on top");

        // The tab after the closed one takes its place...
        assert!(pane.activate(1));
        assert_eq!(pane.close(1).as_ref(), Some(&template("a.java", false)));
        assert_eq!(pane.active_index(), 1);
        assert_eq!(pane.active(), Some(&template("b.java", false)));

        // ...and the one before it when nothing follows.
        assert!(pane.close(1).is_some());
        assert_eq!(pane.active_index(), 0);
        assert_eq!(pane.active(), Some(&PaneItem::Generate));
    }

    #[test]
    fn a_pane_finds_the_generate_tab_and_one_template_by_its_file() {
        let mut pane = Pane::new();
        pane.push(template("model.java", false));
        pane.push(PaneItem::Generate);
        pane.push(template("mapper.xml", true));

        assert_eq!(pane.generate(), Some(1));
    ...
    #[test]
    fn only_an_unsaved_template_blocks_its_own_close() {
        assert!(!PaneItem::Generate.blocks_close());
        assert!(!template("model.java", false).blocks_close());
        assert!(template("model.java", true).blocks_close());
        assert!(
            !PaneItem::Preview {
