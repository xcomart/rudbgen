//! Opening and closing a database session: the tunnel, the JVM, and the
//! session over both.
//!
//! ```text
...
//!
//! [`Session`] is synchronous by design (`rudbgen-jdbc` is deliberately free of
...
//! tunnel between them, reference counted here (rudbman's architecture document §9.3 — `rudbgen-ssh` says in as
...
use rudbgen_ssh::{HostKeyVerifier, PublicKey, SshTunnel, TunnelEvent, TunnelSpec};
...
/// closes the session first and releases the tunnel second — rudbman's §9.3 — whether
...
pub fn build_spec(
    ...
    // The driver's own properties first, the profile's over them: the driver
    // carries what every connection to that product needs — Oracle's
    // `remarksReporting`, MySQL's `useInformationSchema`, SQL Server's
    ...
    // The four custom queries are deliberately *not* handed to the bridge
    // (architecture document, §6): rudbgen runs them itself, through `EXECUTE`
    // on this session, because their contract is jdbgen's — read by label for
    // the two lists, positionally for the two comment queries, with
    // `${catalog}`, `${schema}` and `${table}` substituted on this side. The
    // bridge's own comment enrichment is an internal detail of `DESCRIBE` and
    // answers a differently shaped result, so sending a jdbgen statement into
    // it would read the comment out of a column that is not there.
    spec
}
...
///
/// The order is rudbman's architecture document §9.3 and is not negotiable: the tunnel
...
/// [`Jvm::probe_drivers`](rudbgen_jdbc::Jvm::probe_drivers) and must reach it
...
///
/// The bastion and the target, exactly as rudbman's §9.3 puts it. The authentication
...
impl TunnelLease {
    ...
    ///
    /// The session above it is then dead and stays dead: rudbgen does not
    /// reconnect silently, because a transaction may have been open and the
    /// user has to be the one who decides what happens next.
    ///
...
fn open_tunnel(config: &TunnelConfig, secret: Option<String>) -> Result<TunnelLease, ConnectError> {
    ...
    std::thread::Builder::new()
        .name("rudbgen-tunnel-watch".to_string())
        .spawn(move || {
...
///
/// Refusing rather than asking is the conservative half of rudbman's §9.3's "show the
...
impl StoredHostKeys {
    ...
    fn explain(&self, kind: rudbgen_ssh::SshErrorKind, message: String) -> ConnectError {
        if kind != rudbgen_ssh::SshErrorKind::HostKeyRejected {
        ...
        {
            Ok(rudbgen_core::HostKeyStatus::Mismatch { stored_fingerprint }) => {
                ConnectError::HostKeyMismatch {
...
impl HostKeyVerifier for StoredHostKeys {
    async fn verify(&self, host: &str, port: u16, key: &PublicKey) -> bool {
        let algorithm = rudbgen_ssh::algorithm_name(key);
        let fingerprint = rudbgen_ssh::fingerprint(key);
        let trusted = matches!(
            KnownHosts::load().map(|hosts| hosts.status(host, port, &algorithm, &fingerprint)),
            Ok(rudbgen_core::HostKeyStatus::Trusted)
        );
...
pub(crate) mod h2 {
    ...
    /// The H2 JAR, from `RUDBGEN_TEST_H2_JAR` or the Gradle cache the bridge's
    /// own suite fills.
    pub fn jar() -> PathBuf {
    ...
    /// `rudbgen-jdbc`'s own integration test does — the shape is fixed and this
    ...
    pub fn driver() -> rudbgen_core::DriverDef {
        rudbgen_core::DriverDef {
            id: "h2".to_string(),
            name: "H2".to_string(),
            ...
            ..rudbgen_core::DriverDef::default()
        }
    }

    /// A profile for an in-memory database no other test shares.
    pub fn profile(name: &str) -> rudbgen_core::ConnectionProfile {
        use std::sync::atomic::{AtomicU32, Ordering};
        static SEQ: AtomicU32 = AtomicU32::new(0);
        rudbgen_core::ConnectionProfile::new(
            name,
            "h2",
            format!(
                "jdbc:h2:mem:rudbgen-app-{}",
...
mod tests {
    ...
    /// The four custom queries stay on this side of the bridge.
    ///
    /// rudbgen runs them itself (architecture document, §6, D9) because their
    /// contract is jdbgen's and differs from the one the bridge's own comment
    /// enrichment reads. A statement that leaked into the spec would be read
    /// with the wrong number of columns and quietly answer nonsense.
    #[test]
    fn the_custom_queries_never_reach_the_spec() {
        let profile = ConnectionProfile::new("x", "custom", "jdbc:x", "");
        let driver = DriverDef {
            jars: vec!["/tmp/x.jar".into()],
            custom_queries: rudbgen_core::CustomQueries {
                tables: rudbgen_core::CustomQuery::on("select 1"),
                table_comments: rudbgen_core::CustomQuery::on(
                    "select t, c from meta where s = '${schema}'",
                ),
                column_comments: rudbgen_core::CustomQuery::on("select c, m from cols"),
                ..rudbgen_core::CustomQueries::default()
            },
            ..DriverDef::default()
        };
        let spec = build_spec(&profile, &driver, &Credentials::typed(None, None), "jdbc:x");
        assert_eq!(spec.table_comments_sql, None);
        assert_eq!(spec.column_comments_sql, None);
    }

    /// The driver's properties are the floor and the profile's the ceiling.
    #[test]
    fn a_profile_property_overrules_the_drivers_own() {
        let mut profile = ConnectionProfile::new("x", "oracle-thin", "jdbc:x", "");
        profile
            .props
            .insert("remarksReporting".into(), "false".into());
        profile.props.insert("ssl".into(), "true".into());
        ...
        driver
            .props
            .insert("remarksReporting".to_string(), "true".to_string());
        driver.props.insert(
            "oracle.jdbc.timezoneAsRegion".to_string(),
            "false".to_string(),
        );

        let spec = build_spec(&profile, &driver, &Credentials::typed(None, None), "jdbc:x");
        // The driver's own property, kept.
        assert_eq!(
            spec.props
                .get("oracle.jdbc.timezoneAsRegion")
                .map(String::as_str),
            Some("false")
        );
        // The one both name: the profile wins.
        assert_eq!(
            spec.props.get("remarksReporting").map(String::as_str),
            Some("false")
        );
        // And the profile's own.
        assert_eq!(spec.props.get("ssl").map(String::as_str), Some("true"));
    }
    ...
    fn two_profiles_through_one_bastion_share_a_key() {
        ...
        let with_key = TunnelConfig {
            auth: rudbgen_core::TunnelAuth::Key {
    ...
    fn no_route_to_host_is_recognised_from_either_layer() {
        // The SSH tunnel path: `rudbgen-ssh` surfaces the OS error as text.
