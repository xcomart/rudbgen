...
//!
//! `rudbgen-ui` and gpui, and nothing else (architecture document, §3.1). In
//! particular it does **not** know `rudbgen-jdbc`: the rows arrive through
//! [`GridSource`], so a test source is twenty lines and the crate's tests need
//! no JVM. The same boundary is what lets the grid be pointed at anything with
//! rows and columns — a `DESCRIBE`, a plan, a diff — rather than only at a
//! result set.
//!
//! ## What it holds to
//!
...
///
/// Only key bindings, for now; [`rudbgen_ui::init`] still has to be called for
