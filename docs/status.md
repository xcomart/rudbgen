...
Last updated: 2026-08-23 (M4 done — the template tab, the variable palette, the live preview and the diagnostics; M5 in progress).
...
Test count as of this entry: **1,110 passing** plus the ignored ones, across
...
`rudbgen-gen` (55), `rudbgen-import` (68) and `rudbgen-app` (256), plus the doc
tests. The ones that need a JVM run against a real H2 and are counted above;
the ones that need a server or a keychain are `#[ignore]`d.

## What is next

M0 through M4 are done on Linux: `cargo build --workspace`,
`cargo test --workspace` (1,110 passing), `cargo clippy --workspace
--all-targets -- -D warnings` and `cargo fmt --all --check` are all green, and
`cargo run -p rudbgen-app` opens the sample H2 connection, generates against
it, and edits a template with the live preview beside it — see
`docs/screenshots/welcome.png`, `explorer.png`, `generate.png`,
`summary.png`, `template-editor.png` and `completion.png`. What closes each
milestone formally is the same suite green on CI's other two platforms, which
only CI can say.

Two things earlier milestones deliberately left for later, written down so they
are not mistaken for oversights:

- The welcome screen's **Import from jdbgen…** and the Generate tab's
  **Rules…** are drawn disabled with a tooltip that says so. Both are M5, whose
  pure half — `crates/rudbgen-import/` and `rudbgen-core`'s
  `AbbreviationStore` — is already in the tree.
...
Two things M4 itself left, for the same reason:
...
- The screenshots in `docs/screenshots/` are captures of the whole window
  *surface*, which under client-side decorations carries a 12 px transparent
  band for the drop shadow all round. A capture that does not crop that band
  composites whatever is behind the window into it — which is what the strip of
  noise below the status bar in `generate.png` and `summary.png` is. It is not
  the application: crop `SHADOW_BAND` from each edge of an RGBA capture and the
  frame is clean.
- Open question 1 of the architecture document — whether a template's output
  name may live in a front-matter header in the file itself, rather than in a
  row of the set — was to be decided in M4 at the latest, and is decided
  **no**: the name is rendered per table with the same engine as the body, the
  ...
  unreadable to jdbgen for no gain the tab does not already give.

M5 is next: the jdbgen import wizard, the abbreviation rules dialog, and the
custom-query Test button's remaining half.
