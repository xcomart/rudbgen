...
//!
//! Six themes ship with rudbgen. A theme can also come from a file:
...
//! rudbgen reads unchanged — has to keep loading, so the grid slots are
...
pub struct ThemeEntry {
    ...
    /// Whether the palette is a dark one.
    pub dark: bool,
    /// Whether the theme ships with rudbgen rather than coming from a file.
    pub builtin: bool,
}
...
impl ThemeRegistry {
    ...
    /// Whether `id` names a theme that ships with rudbgen.
...
/// rudbgen does not know are ignored, and a color it cannot parse falls back to
...
mod tests {
    ...
    /// The guarantee reaches themes rudbgen never saw: a file carries no `icon`
