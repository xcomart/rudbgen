...
/// known (rudbman's architecture document, §9.3).
...
impl SshTunnel {
    ...
    pub fn open(
        ...
        let thread_name = format!("rudbgen-ssh-{}-{}", spec.host, spec.port);
    ...
    loop {
        ...
        match step {
            ...
            Step::Liveness => {
                ...
                // the user has to be told (rudbman's architecture document, §9.3).
                if handle.is_closed() {
                    return Ending::Failed(
                        SshErrorKind::Io,
                        "the connection to the bastion was lost".to_owned(),
