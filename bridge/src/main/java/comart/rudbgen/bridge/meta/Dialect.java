 ...
 * needs.
 *
 * <p>Table and column comments cannot be answered through JDBC alone: several
 * drivers report none where the server holds one, and a few report a
 * placeholder where the server holds none ({@link Comments}). Reconciling that
 * needs a vendor catalogue query, and a vendor catalogue query needs to know the
 * vendor.
 *
...
public enum Dialect {
    ...
    /** MySQL. */
    MYSQL,
    /** MariaDB. Reported separately from MySQL because its catalogue differs. */
    MARIADB,
    /** Oracle Database. */
    ORACLE,
