//! Operation codes and the response envelope (the bridge's `README.md`,
//! "Operations" and "Response envelope").
//!
...
/// A bridge operation.
///
/// The `handle` argument means a session or a cursor depending on the
/// operation; see each variant.
///
/// rudbman's `LobRead` (`0x25`) and `JobStart` / `JobPoll` / `JobCancel`
/// (`0x40`–`0x42`) are absent: rudbgen never ferries row data, so the bridge
/// has neither the LOB path nor the job layer (architecture.md D3). **The codes
/// are retired, not free** — a new operation here takes a new number, so that
/// the two projects' op tables keep lining up.
...
mod tests {
    ...
    fn operation_codes_match_the_table() {
        // Straight from the bridge's `README.md`; a renumbering here would be
        ...
        // 0x25 and 0x40..0x42 are rudbman's, retired here and never reassigned:
        // no variant may claim one of them.
        for op in [
            Op::OpenSession,
            Op::CloseSession,
            Op::Ping,
            ...
            assert!(
                !matches!(op.code(), 0x25 | 0x40..=0x42),
                "{op:?} claims a retired code"
            );
        }
    }
