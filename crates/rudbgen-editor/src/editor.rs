...
//! wrong text. The single-line field in `rudbgen-ui` converts by walking the
//! string; this one cannot, and [`crate::buffer`] explains what it does
//! instead.
//!
//! # No `DisplayMap`
//!
//! `rudbgen-ui`'s [`TextInput`] keeps a `DisplayMap` between "the bytes stored"
...
use rudbgen_ui::scrollbar::{
    DraggedThumb, Scrollbar, ScrollbarAxis, ScrollbarState, hide_later, hide_now,
};
use rudbgen_ui::{Checkbox, TextInput, editor_theme, theme};
...
///
/// Call once during application start-up, after `rudbgen_ui::init`. Everything
