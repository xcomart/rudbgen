...
//!
//! `tests/containers.rs` reads `remarks` off five server products, all of
//! which *can* be commented, so it never sees the edge of the contract. SQLite
//! cannot be commented at all: it has no `COMMENT ON`, no `COMMENT=` clause, no extended
...
//!
//! ```text
//! export RUDBGEN_TEST_SQLITE_URL="jdbc:sqlite:$(mktemp -u)/rudbgen.db"
//! cargo test -p rudbgen-jdbc --test sqlite
//! ```
//!
...
//! and overridable with `RUDBGEN_TEST_SQLITE_JAR`. A missing JAR is a panic
...
/// database file is.
const URL_VAR: &str = "RUDBGEN_TEST_SQLITE_URL";
...
fn driver_jar() -> PathBuf {
    if let Some(path) = std::env::var_os("RUDBGEN_TEST_SQLITE_JAR") {
        let path = PathBuf::from(path);
        assert!(
            path.is_file(),
            "RUDBGEN_TEST_SQLITE_JAR points at {}, which is not a file",
            path.display()
        );
        return path;
    }
    find_in_gradle_cache("org.xerial", "sqlite-jdbc").unwrap_or_else(|| {
        panic!(
            "the org.xerial:sqlite-jdbc driver JAR was not found.\n\
             ...
             \n\
             or point RUDBGEN_TEST_SQLITE_JAR at a JAR you already have."
        )
    })
}
...
impl Db {
    /// Opens the database `RUDBGEN_TEST_SQLITE_URL` names, or answers `None`
...
///
/// A thing nobody commented comes back as one of three values depending on the
/// product — SQL NULL, the empty string, or whitespace — and none of the three
/// is a comment. Which one a driver picks is its habit, and a test that
/// asserted one of them would be asserting the habit.
