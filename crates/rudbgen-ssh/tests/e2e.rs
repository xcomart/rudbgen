...
//!
//! ```text
//! export RUDBGEN_SSH_TEST_HOST=bastion.example.com   # required
//! export RUDBGEN_SSH_TEST_PORT=22                    # optional, defaults to 22
//! export RUDBGEN_SSH_TEST_USER=ops                   # optional, defaults to $USER
//! export RUDBGEN_SSH_TEST_PASSWORD=...               # one of these two; without
//! export RUDBGEN_SSH_TEST_KEY=~/.ssh/id_ed25519      # either, the agent is used
//! export RUDBGEN_SSH_TEST_REMOTE_HOST=db.internal    # optional, defaults to localhost
//! export RUDBGEN_SSH_TEST_REMOTE_PORT=22             # optional, defaults to 22
//!
//! cargo test -p rudbgen-ssh --test e2e -- --ignored --nocapture
//! ```
//!
...
fn losing_the_bastion_is_reported_as_an_error_and_not_repaired() {
    ...
    // (rudbman's architecture document, §9.3).
    assert!(!tunnel.is_alive());
    assert!(
        TcpListener::bind((Ipv4Addr::LOCALHOST, port)).is_ok(),
        "port {port} was not released"
...
#[test]
#[ignore = "needs a real SSH server; see the module docs for RUDBGEN_SSH_TEST_*"]
fn a_real_bastion_forwards_a_real_port() {
    let Ok(host) = std::env::var("RUDBGEN_SSH_TEST_HOST") else {
        panic!("set RUDBGEN_SSH_TEST_HOST; see the module docs");
    };
    let port = env_u16("RUDBGEN_SSH_TEST_PORT", 22);
    let username = std::env::var("RUDBGEN_SSH_TEST_USER")
        .or_else(|_| std::env::var("USER"))
        .expect("set RUDBGEN_SSH_TEST_USER");
    let auth = match (
        std::env::var("RUDBGEN_SSH_TEST_PASSWORD"),
        std::env::var("RUDBGEN_SSH_TEST_KEY"),
    ) {
        (Ok(password), _) => SshAuth::Password(password),
        (_, Ok(path)) => SshAuth::PrivateKeyFile {
            path: path.into(),
            passphrase: std::env::var("RUDBGEN_SSH_TEST_KEY_PASSPHRASE").ok(),
        },
        _ => SshAuth::Agent,
    };
    let remote_host =
        std::env::var("RUDBGEN_SSH_TEST_REMOTE_HOST").unwrap_or_else(|_| "127.0.0.1".to_owned());
    let remote_port = env_u16("RUDBGEN_SSH_TEST_REMOTE_PORT", 22);
