...
//!
//! The transport never decides on its own whether a bastion is trustworthy: it
...
//! application layer — [`rudbgen_core::KnownHosts`] already owns that file, and
//! deciding what to do about an unknown or changed key means asking the user,
//! which is a UI concern. This module only ships the two trivial policies that
//! are useful for tests and for bootstrapping.

use russh::keys::{HashAlg, PublicKey};

/// Decides whether the host key presented by a bastion should be trusted.
///
...
/// It is not the default anywhere — [`SshTunnel::open`](crate::SshTunnel::open)
/// takes the policy as an argument precisely so that choosing this one is
/// always a visible decision at the call site.
