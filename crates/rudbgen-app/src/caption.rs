...
//! platform.
//!
//! On Windows, a window opened with [`TitlebarStyle::System`] keeps the
//! standard DWM caption, so the title text and the minimise / maximise / close
//! glyphs are painted by Windows, not by us. Two things make that go wrong:
//!
...
//! costs nothing here — rudbgen has one theme at a time, for the one window it
//! owns. It matters under the custom title bar too, where AppKit still draws
//! the traffic lights over the toolbar band.
//!
//! Nothing on Linux: the caption there is the compositor's, themed by the
//! desktop rather than by the window, and when rudbgen draws its own it draws
//! it out of the same palette as the rest of the chrome.
//!
//! [`TitlebarStyle::System`]: rudbgen_core::TitlebarStyle

use gpui::{App, Window};
use rudbgen_ui::Theme;
