//! JVM bootstrap and the one JNI call this crate makes (the bridge's
//! `README.md`, "Build" and "Entry point").
//!
//! # One JVM per process
//!
...
const BRIDGE_CLASS: &str = "comart/rudbgen/bridge/Bridge";

/// Environment variable that overrides where the Java runtime is looked for.
pub const JAVA_HOME_ENV: &str = "RUDBGEN_JAVA_HOME";

/// Environment variable that overrides where the bridge JAR is looked for.
pub const BRIDGE_JAR_ENV: &str = "RUDBGEN_BRIDGE_JAR";
...
impl JvmConfig {
    ...
    /// Picks the Java runtime: an explicit setting, then a runtime bundled
    /// beside the executable, then `RUDBGEN_JAVA_HOME`, then `JAVA_HOME`.
    ///
...
/// The bundled runtime beside the executable, in the order it is looked for:
/// `<exe_dir>/runtime` — the flat archive Windows and Linux ship — then
/// `<exe_dir>/../runtime`, which on macOS is `<bundle>/Contents/runtime`.
...
///
/// In order: the `RUDBGEN_BRIDGE_JAR` environment variable, `lib/` beside the
...
impl Jvm {
    ...
    /// not matter (the bridge's `README.md`, "Concurrency").
...
fn create(options: Vec<String>, java_home: Option<PathBuf>, jar: &Path) -> Result<Jvm> {
    ...
    vm.attach_current_thread(|env| {
        ...
        // the error messages.
        let class = env
            .find_class(jni_str!("comart/rudbgen/bridge/Bridge"))
            .map_err(|source| {
                Error::JvmStart(format!(
...
mod tests {
    ...
    fn the_fixed_options_are_all_there() {
        let options = JvmConfig::new("/tmp/rudbgen-bridge.jar")
            .with_heap_mb(2048)
            .options();
        assert_eq!(options[0], "-Djava.class.path=/tmp/rudbgen-bridge.jar");
    ...
    fn the_bridge_jar_is_looked_for_beside_the_executable_then_above_it() {
        ...
        assert_eq!(
            candidates,
            vec![
                PathBuf::from("/Applications/rudbgen.app/Contents/MacOS/lib/rudbgen-bridge.jar"),
                PathBuf::from("/Applications/rudbgen.app/Contents/MacOS/rudbgen-bridge.jar"),
                PathBuf::from("/Applications/rudbgen.app/Contents/lib/rudbgen-bridge.jar"),
            ],
    ...
    fn the_bundled_runtime_is_looked_for_beside_the_executable_then_above_it() {
        let candidates = runtime_candidates(Path::new("/opt/rudbgen/bin/rudbgen")).unwrap();
        assert_eq!(
            candidates,
            vec![
                PathBuf::from("/opt/rudbgen/bin/runtime"),
                PathBuf::from("/opt/rudbgen/runtime"),
            ],
