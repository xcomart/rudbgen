//! Decoder for the `RDB1` result batch format (the bridge's `README.md`, "RDB1 batch codec").
//!
//! ```text
//! Batch  := Header Column*
...
pub enum ColumnKind {
    ...
    Bin,
    // Byte 6 was rudbman's `Lob`, `row_count` pairs of `(u64 lob_id, u64 size)`
    // resolved by a follow-up `LOB_READ`. Both are retired (architecture.md D3)
    // and the number is never reused, so a batch cannot mean one thing in
    // rudbman and another here; the bridge now sends a `BLOB` as `Bin` and a
    // `CLOB` as `Str`. A 6 on the wire is an
    // [`CodecError::UnknownKind`](CodecError::UnknownKind) — which is what
    // reading a rudbman batch by mistake should be.
}
...
pub enum Value<'a> {
    ...
    Str(&'a str),
    /// Raw bytes. Also carries a `BLOB`, which this bridge materialises.
    Bin(&'a [u8]),
}
...
mod tests {
    ...
    #[test]
    fn the_retired_lob_kind_is_not_decoded_as_anything() {
        // Byte 6 was rudbman's LOB reference column. It is retired here and the
        // number is not reused, so a batch that carries one -- a rudbman bridge
        // on the other end of the JNI call -- has to fail loudly rather than be
        // read as some other shape.
        let bytes = Encoder::new(1, true)
            .column(6, &[true], &[0u8; 16])
            .finish();
        assert!(matches!(
            Batch::decode(&bytes),
            Err(CodecError::UnknownKind { column: 0, kind: 6 })
        ));
    }
