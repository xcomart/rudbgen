...
//! written by a newer rudbgen opens here and is saved back without losing the
...
/// What a generation run does when the file it is about to write is already
/// there.
///
/// The default for a new run; the generation dialog may still override it for
/// one run without changing the setting. See the architecture document, D11.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize, Default)]
#[serde(rename_all = "snake_case")]
pub enum OverwritePolicy {
    /// Replace the existing file.
    Overwrite,
    /// Leave the existing file alone and count the pair as skipped.
    Skip,
    /// Stop at the first conflict and let the user decide, per file.
    ///
    /// The default, because the other two are each destructive in one
    /// direction: `Overwrite` loses hand edits, `Skip` silently produces an
    /// output tree that does not match the schema it claims to.
    #[default]
    Ask,
}

/// Read an [`OverwritePolicy`] leniently, falling back to the default.
///
/// The three names are what this build writes, but `settings.json` is
/// hand-editable and predates the field in every file written before it
/// existed. A missing key, an explicit `null`, and a fourth word somebody
/// invented all load as [`OverwritePolicy::Ask`] rather than failing the whole
/// file — the same forgiveness the rest of this module applies to numbers it
/// clamps. It goes through [`serde_json::Value`] because a `Deserializer` is
/// consumed by the attempt: there is no way to try a variant and put the input
/// back.
fn deserialize_overwrite_policy<'de, D>(deserializer: D) -> Result<OverwritePolicy, D::Error>
where
    D: Deserializer<'de>,
{
    let Some(value) = Option::<serde_json::Value>::deserialize(deserializer)? else {
        return Ok(OverwritePolicy::default());
    };
...
pub enum TitlebarStyle {
    /// rudbgen draws it: the toolbar doubles as the title bar. The default.
    #[default]
    Custom,
...
/// Everything rudbgen persists in `settings.json`.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(default)]
pub struct AppSettings {
    ...
    /// tags rudbgen actually ships translations for lives in the app layer, so
    ...
    /// the architecture document, §4.6.
    pub jvm_heap_mb: u32,
    /// Extra JVM arguments, appended after the ones rudbgen sets itself.
    ///
    ...
    /// What a generation run does with a file that already exists.
    ///
    /// The starting value of the generation dialog's own switch; a run may
    /// differ from it without changing the setting.
    #[serde(deserialize_with = "deserialize_overwrite_policy")]
...
mod tests {
    ...
    #[test]
    fn sanitize_bounds_the_jvm_heap_and_the_explorer_width() {
        let mut settings = AppSettings {
            jvm_heap_mb: 0,
            explorer_width: 0.0,
            ..AppSettings::default()
        ...
        assert_eq!(settings.explorer_width, 140.0);

        settings.explorer_width = 5_000.0;
        settings.sanitize();
        assert_eq!(settings.explorer_width, 720.0);
    ...
    #[test]
    fn the_overwrite_policy_serializes_in_snake_case_and_defaults_to_ask() {
        assert_eq!(
            serde_json::to_value(OverwritePolicy::Skip).unwrap(),
            serde_json::json!("skip")
        );
        assert_eq!(
            serde_json::from_str::<OverwritePolicy>("\"overwrite\"").unwrap(),
            OverwritePolicy::Overwrite
        );
        assert_eq!(OverwritePolicy::default(), OverwritePolicy::Ask);
    }

    /// A settings file written before the policy existed — or by a hand that
    /// mistyped it — must still open, on the safe side of the three.
    #[test]
    fn an_absent_or_unreadable_overwrite_policy_falls_back_to_ask() {
        let dir = tempfile::tempdir().expect("tempdir");
        let path = dir.path().join("settings.json");

        fs::write(&path, br#"{"theme":"one-light"}"#).expect("write");
        assert_eq!(
        ...
        // A word this build does not know, and an explicit null, are the same
        // thing as not having chosen: one mistyped value must not cost the
        // user the rest of `settings.json`.
        for text in [
            br#"{"overwrite_policy":"whatever"}"#.as_slice(),
            br#"{"overwrite_policy":null}"#.as_slice(),
        ] {
