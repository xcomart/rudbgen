...
//! `rudbgen` service namespace, so neither `connections.json` nor a profile
...
//! the tunnel. See the architecture document, §5.
//!
...
/// Service namespace used for every credential rudbgen stores.
const SERVICE: &str = "rudbgen";
...
/// keychain.
const INIT_PROBE_ACCOUNT: &str = "__rudbgen_store_probe__";
...
mod tests {
    ...
    fn the_service_namespace_is_the_app_name() {
        // The architecture document spells the keychain key `rudbgen:<id>`.
        assert_eq!(SERVICE, "rudbgen");
    }
