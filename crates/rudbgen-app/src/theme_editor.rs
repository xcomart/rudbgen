...
//! `rudbgen-ui`'s loader: the loader is forgiving because a broken file in the
...
pub struct CatalogEntry {
    ...
    /// Human-readable name.
    pub name: String,
    /// Whether the entry ships with rudbgen rather than coming from a file.
    pub builtin: bool,
}
...
impl CatalogFile {
    ...
    /// is usually somewhere rudbgen will never read from again.
    ///
    /// # Errors
    ///
...
impl Catalog {
    ...
    pub fn directory(self) -> Result<PathBuf> {
        match self {
            Self::UiTheme => rudbgen_core::ui_themes_dir(),
            Self::EditorTheme => rudbgen_core::editor_themes_dir(),
        }
    }
...
mod tests {
    ...
    use rudbgen_ui::{EditorTheme, Theme};
    ...
    /// `rudbgen_ui::theme_store`'s own tests, once per format.
    #[test]
