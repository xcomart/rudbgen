...
//! with, and what a metadata query asks for.
//!
...
//! same discipline `rudbgen-core` applies to a stored profile, repeated here
...
pub struct DescribeRequest {
    ...
    ///
    /// That is the whole set. rudbman also answered `ddl`, `procedures`,
    /// `functions` and `sequences`; a code generator writes files from tables,
    /// views, columns, keys and indexes, so those went with the vendor
    /// catalogue queries they needed (architecture.md D3). An unknown kind is a
    /// `protocol` error from the bridge, not a silent empty list.
    pub kind: String,
    /// Exact catalog name.
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// Mirrors the rules `rudbgen-core` applies to a stored profile — that
...
mod tests {
    ...
    fn debug_hides_the_password_the_props_and_the_url_credentials() {
        let spec = ConnectionSpec::new("jdbc:postgresql://db:5432/app?password=hunter2", "org.pg")
            ...
            .with_prop("ApplicationName", "rudbgen");
